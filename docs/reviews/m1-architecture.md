...
| `app/services/lead_service.py:25-43` | Global `_repo` and `_redis_client` singletons with lazy init | C | **FIXED in commit `9cb26b5`** — replaced with Depends()-based dependency injection, following the `app/dependencies/auth.py` `get_current_user` DI pattern. The `_repo`/`_redis_client` mutable globals were removed from `lead_service`; the shared state now lives as provider functions `get_lead_repo()`/`get_redis()` in `app/dependencies/services.py`. `lead_service` keeps thin backward-compatible wrappers (`_get_or_create_repo`, `_get_redis`); all public functions accept an optional `repo`/`redis` parameter; `app/routers/leads.py` endpoints inject `repo: LeadRepository = Depends(get_lead_repo)` and pass it through. Structural only — no behavior change. |
| `app/services/widget_service.py:36-41` | Global `_redis_client` singleton | C | **FIXED in commit `6585a96`** — same DI pattern as `9cb26b5`. Module-level `_repo`/`_redis_client` removed from `widget_service`; repo selection (postgres vs in-memory) and the redis client now live in `app/dependencies/services.py` (`get_widget_repo()`, `get_redis()`). `app/routers/widgets.py` and the four `leads.py` endpoints that call `widget_service.get_widget` inject `repo: WidgetRepositoryProtocol = Depends(get_widget_repo)`. `widget_service` keeps `_get_repo()` as a backward-compatible wrapper. Structural only — no behavior change. |
...
| **C** (flag only) | **19** | +3 from re-tiered M1 B items (#4, #7, #8); +1 new (unbounded fallback dict) | **M17 disposition:** 6 CLOSED — accepted (§5.5/§8.5/§8.9/§9/§5.4 + tenant_id); 1 CLOSED — plan update (400→422); 4 REJECTED — Won't Fix (naming); 2 DEFERRED — dual-repo (trigger: third repo). **M18 (commit `d6fb7c3`):** re-enrich race condition → FIXED. **M19 (commits `c3a96e3`, `a7f9ed2`):** repository protocol conformance → FIXED (`LeadRepositoryProtocol`/`WidgetRepositoryProtocol` + 3 conformance tests); unbounded fallback dict → FIXED (`OrderedDict` + LRU eviction, `_MAX_IN_PROCESS_KEYS` = 10 000, 3 tests). **M20 (commits `9cb26b5`, `6585a96`):** global singletons → FIXED (Depends()-based DI; provider functions `get_lead_repo()`/`get_widget_repo()`/`get_redis()` in `app/dependencies/services.py`, pattern from `app/dependencies/auth.py`). **M21 (commits `babf92b`, `ea5c738`):** implemented the confirmation webhook side-effect — the last core Definition-of-Done item from the capstone brief, previously flagged absent as Tier C Category 9 in `m11-test-gap-audit` / `m13-gap-fill`. Optional `config.webhook_url` validated as HTTPS in `WidgetCreate`; new `app/services/webhook_service.dispatch_webhook()` (fail-open, 3s `asyncio.wait_for`, never raises); fire-and-forget dispatch in `lead_service.submit_lead`, skipped for honeypot/no-URL; follow-up fix retaining a strong reference to in-flight tasks (`_webhook_tasks` set + `add_done_callback` discard) so tasks can't be GC'd mid-execution. **M23 (commit `c95b4a7`):** closed the two long-flagged M3 performance-domain Tier C items — `app/repositories/postgres_lead_repo.py` provides a full Postgres-backed `LeadRepository` (protocol-conformant), and `batch_delete` now issues a single `DELETE WHERE id = ANY($1::uuid[])` with tenant isolation instead of N individual deletes. Wired via `get_lead_repo()` DI and `lead_worker._get_worker_repo()` when Postgres is enabled. **Remaining open: 0** — all 19 Tier C items are fixed, closed, rejected, or deferred. |
