...
| **C** (flag only) | **19** | +3 from re-tiered M1 B items (#4, #7, #8) | **M17 disposition:** 6 CLOSED — accepted (§5.5/§8.5/§8.9/§9/§5.4 + tenant_id); 1 CLOSED — plan update (400→422); 4 REJECTED — Won't Fix (naming); 2 DEFERRED — dual-repo (trigger: third repo). **Remaining open: 4** (re-enrich race condition, repository protocol conformance, global singletons, in-process rate-limit dict growth) |
