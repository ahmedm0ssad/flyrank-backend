...
|---|---------|---------|--------|
| 2 | `.env.example` missing `IPINFO_TOKEN` | **Fixed** — added `IPINFO_TOKEN=your_ipinfo_token` to `.env.example`. | `e6b9fbd` |
...
| 2 | `.env.example:1-7` | **Missing `IPINFO_TOKEN`** — `app/services/geo_service.py:70` reads `IPINFO_TOKEN` via `os.getenv("IPINFO_TOKEN", "")` but this variable is not listed in `.env.example`. Anyone setting up from the example will silently skip the ipinfo.io geo provider, degrading enrichment reliability. | B | **Fixed** in commit `e6b9fbd`. `IPINFO_TOKEN=your_ipinfo_token` added to `.env.example`. |
